...
static struct platform_device * gpiodev_platform_device;
...
gpio_mod_init(void)
{
	int ret;
        ...
        gpiodev_platform_device = platform_device_alloc("GPIODEV", -1);
        if (!gpiodev_platform_device)
                return -ENOMEM;

        ret = platform_device_add(gpiodev_platform_device);
        if (ret < 0) {
		printk(KERN_INFO DRVNAME ": Error platform_device_add()\n");
                platform_device_put(gpiodev_platform_device);
                return ret;
        }
	ret = platform_driver_register(&gpio_driver);
	if (ret) {
		printk(KERN_INFO DRVNAME ": Error registering platfom driver!\n");
		platform_device_unregister(gpiodev_platform_device);
	}
